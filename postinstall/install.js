    ...
    if (nest && nest.length > 0) {
      ...
      if (fs.existsSync(fileLoc)) {
        ...
        if (config && typeof config === "string") {
          ...
          if (contents && contents.localizedResources && !contents.localizedResources.FaganscSPFXFormControlStrings) {
            contents.localizedResources["FaganscSPFXFormControlStrings"] = "node_modules/fagansc-spfx-form-controls/lib/loc/{locale}.js";
            // Update the file
            fs.writeFileSync(fileLoc, JSON.stringify(contents, null, 2));
            console.log("INFO: Localized resource added.");
          } else {
            console.warn(`WARNING: it seems something is wrong with the config.json file or the "FaganscSPFXFormControlStrings" reference was already set.`);
          }
        } else {
